...
mod = 10**9 + 7 # модуль, по которому выполняются все операции. должно быть большим и взаимно простым с p (опционально вообще простым)
...
### Поиск суммы на отрезке + изменение значения в массиве. Постановка задачи, решение за O(logN) при помощи дерева отрезков. Построение дерева отрезков. Оценка времени выполнения.
 ...
 _см [segment tree(sum)](https://github.com/vasslin/Algoritms-and-Data-structures/blob/main/segment%20tree%20(sum))_
...
## 11 Вопрос.
### Система непересекающихся множеств. Описание структуры. Эвристика по размеру. Эвристика сжатия пути. Асимптотика операций.
...
**Асимптотика:**
+ Эвристика сжатия путей улучшает асимптотику до O(logn) в среднем. Используется именно амортизированная оценка — понятно, что в худшем случае нужно будет сжимать весь бамбук за O(n).
+ Эвристика по размеру ограничивают высоту дерева до O(logn), а, соответственно, и асимптотику нахождения корня тоже.

При использовании эвристики сжатия плюс эвристики по размеру асимптотика будет O(a(n)), где a(n) — обратная функция Аккермана (очень медленно растущая функция, для всех адекватных чисел не превосходящая 4).



## 12 Вопрос.
### Алгоритм Краскала. Описание алгоритма. Применение системы непересекающихся множеств. Асимптотика.

**Алгорит Краскала** - алгоритм поиска минимального остовного поддерева в графе (жадный алгоритм).

_Описание алгоритма:_
Задача: добавить n-1 ребро с минимальными весами для создания одной компненты связности (из первоначальных n)

+ сортируем ребра по увеличению веса
+ перебираем ребра (from, to, weighr):

    если вершины from и to лежат в разных компнентах: добавляем ребро
...
**Двоичное дерево поиска** - дерево, для которого выполняются свойства:

+ каждая вершина имеет не более двух детей, разность между гглубинами листьев не более 1 (бинарное дерево)
+ все вершины имеют ключи (значения), по которым происходит сравнение
+ все вершины _левого поддерева v_ имеют ключ, значение которого не больше v
+ все вершины _правого поддерева v_ имеют ключ, значение которого больше v
+ _левое_ и _правое_ поддеревья v - двоичные деревья поиска

**Построение (build)**
Используем функцию **insert** поочередно для всех элементов.

**Добавление элемента**
Функцию шагает по дереву и смотрит на текущее значение элемента в вершине, сравниваем его с el и направаляет дальше (в левого или правого ребенка). Если ребенок == null, создаем нового ребенка от этой вершины со значением el.

_псевдокод_
**def insert(Node p, int el)**{
  if p == null {
    p = new Node(el, null, null);
    return;
    }
  if (p.value >= el){
    insert(p.left, el);
...
_Идея_: 
+ находим ЛистЛевый - самый левый элемент правого поддерева - минимальный элемент, больший v (или ЛистПравый - самый правый элемент левого поддерева - макс элемент, меньший v)
+ меняем их местами (значения вершин)
+ удаляем ЛистЛевый/ЛистПравый


**Асимптотика:**
+ insert - в лучшем случае O(logN), в худшем случае - O(N)
+ find, erase - в лучшем случае O(1), в худшем случае - O(N)
+ build -  в лучшем случае O(N*logN), в худшем случае - O(N**2)


## 14 вопрос.
### Декартово дерево. Построение, добавление элемента, удаление элемента, split, merge. Пример каждой операции. Асимптотика операций.
